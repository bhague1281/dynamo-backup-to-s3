...
DynamoBackup.prototype._copyTable = function (tableName, itemsReceived, callback) {
    ...
    ddb.describeTable({ TableName: tableName }, function (err, data) {
        ...
        const maxCapacityUnits = Math.max((Table.ProvisionedThroughput.ReadCapacityUnits * self.readPercentage), 1);
        const averageItemSize = Table.TableSizeBytes / Table.ItemCount;

        self._streamItems(tableName, null, 0, maxCapacityUnits, averageItemSize, itemsReceived, callback);
    });
};

DynamoBackup.prototype._streamItems = function fetchItems(
    tableName, startKey, remainingCapacityUnits, maxCapacityUnits, averageItemSize, itemsReceived, callback
) {
    var self = this;
    var ddb = new AWS.DynamoDB();
    ...
    if (usableCapacity > 0) {
        ...
        ddb.scan(params, function (err, data) {
            ...
            const nextRemainingCapacity = usableCapacity - data.ConsumedCapacity.CapacityUnits;
            const timeout = Math.max(1000 - (new Date().getTime() - startOfSecond), 0);
